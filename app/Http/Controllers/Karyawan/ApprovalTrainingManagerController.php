...
class ApprovalTrainingManagerController extends Controller
{
    /**
    ...
    {
        $params['data'] = \App\Training::where('approved_manager_id', \Auth::user()->id)->orderBy('id', 'DESC')->get();
        $params['data_biaya'] = \App\Training::where('approved_manager_id', \Auth::user()->id)->where('is_approve_manager_actual_bill', 0)->where('status_actual_bill', 2)->get();

        return view('karyawan.approval-training-manager.index')->with($params);
    }
    ...
    public function proses(Request $request)
    {
        ...
        $training->is_approved_manager       = $request->status;
        $training->date_approved_manager     = date('Y-m-d H:i:s');
        ...
        return redirect()->route('karyawan.approval.training-manager.index')->with('message-success', 'Data Successfully processed !');
    }
    ...
    public function prosesBiaya(Request $request)
    {
        ...
        {
            $data->is_approve_manager_actual_bill = 0;
    ...
    public function biaya($id)
    {
        ...
        return view('karyawan.approval-training-manager.biaya')->with($params);
    }
    ...
    {   
        ...
        return view('karyawan.approval-training-manager.detail')->with($params);
    }
}
